...
class TopicAnalyzer:
    ...
    def __init__(self, custom_keywords: Dict[str, Dict] = None):
        # Load topic patterns from DB (with 5-min TTL cache); fall back to hardcoded defaults.
        # Deep-copy so per-call mutations (custom_keywords merge) don't poison the cached dict.
        self.topic_patterns = copy.deepcopy(
            _pl.get("topic_patterns", _DEFAULT_TOPIC_PATTERNS)
        )
