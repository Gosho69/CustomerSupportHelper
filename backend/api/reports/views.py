...
class GenerateReportView(APIView):
    ...
    def post(self, request):
        ...
        # Refuse to generate a report for a period that ended before the agent
        # was created — there cannot be any meaningful data in that window.
        if end_date < agent.created_at.date():
            return Response(
                {
                    'error': (
                        f'Cannot generate a report for {start_date}–{end_date}: '
                        f'the period ended before this agent was created '
                        f'({agent.created_at.date()}).'
                    )
                },
                status=status.HTTP_400_BAD_REQUEST
            )
